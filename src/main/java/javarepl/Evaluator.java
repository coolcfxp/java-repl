...
public class Evaluator {
  ...
  public Either<Throwable, Evaluation> evaluate(final String expr) {
    return parseExpression(expr).flatMap(new Mapper<Expression, Either<Throwable, Evaluation>>() {
              public Either<Throwable, Evaluation> call(Expression expression) throws Exception {
                ...
                if (resultForValue.isLeft() && resultForValue.left() instanceof ExpressionCompilationException
                        && expression instanceof Value) {
                  Either<Throwable, Evaluation> resultForStatement = evaluate(new Statement(expr));
                  return resultForStatement.isLeft() && resultForStatement
                          .left() instanceof ExpressionCompilationException ? Left.<Throwable, Evaluation>left(
                          new ExpressionCompilationException(
                                  sequence(resultForStatement.left().getMessage(), resultForValue.left().getMessage())
                                          .unique().toString("\n\n"))) : resultForStatement;
                }
  ...
  private Either<Throwable, Expression> createAssignmentWithType(String expression) {
    try {
      ...
      java.lang.reflect.Method declaredMethod =
              detectMethod(match.group(1) + " " + randomIdentifier("method") + "(){}");
      return right(
              (Expression) new AssignmentWithType(expression, declaredMethod.getGenericReturnType(), match.group(2),
                      match.group(3)));
    }
    catch (Exception e) {
  ...
  private Either<Throwable, Evaluation> evaluate(Type expression) {
    ...
    try {
      File outputPath = directory(context.outputDirectory(),
              expression.typePackage().getOrElse("").replace('.', File.separatorChar));
  ...
  private Either<Throwable, Expression> createMethodExpression(String expression) {
    try {
      ...
      return right((Expression) new Method(expression, declaredMethod.getGenericReturnType(), declaredMethod.getName(),
              sequence(declaredMethod.getParameterTypes())));
    }
    catch (Exception e) {
      return left(Utils.unwrapException(e));
    }
  ...
  private Class detectClass(String expression) throws Exception {
    ...
    final String sources =
            renderMethodSignatureDetection(context, className, "public " + expression + " detectClass(){return null;}");
    Files.write(sources.getBytes(), outputJavaFile);
            ...
            .reduceLeft(new Reducer<Field, Sequence<Result>>() {
              public Sequence<Result> call(Sequence<Result> results, Field field) throws Exception {
                Option<Result> result =
                        result(field.getName()).filter(where(value(), not(equalTo(field.get(expressionInstance)))));
  ...
  private String nextResultKeyFor(Expression expression) {
    return (expression instanceof Value) ? context.nextResultKey() : expression.key();
  }

  private Option<java.lang.reflect.Type> typeFor(Expression expression) {
    return (expression instanceof AssignmentWithType) ? Option.some(((AssignmentWithType) expression).type())
            : Option.<java.lang.reflect.Type>none();
  }

  private void compile(File file) throws Exception {
    String classpath =
            sequence(System.getProperty("java.class.path")).join(sequence(classLoader.getURLs()).map(urlAsFilePath()))
    ...
    JavaCompiler.CompilationTask task =
            compiler.getTask(null, fileManager, diagnostics, sequence("-cp", classpath), null, compilationUnits);
