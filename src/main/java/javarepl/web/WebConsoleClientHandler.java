...
public final class WebConsoleClientHandler {
  ...
  private void createProcess() {
    if (port.isEmpty()) {
      try {
        ...
        ProcessBuilder builder = new ProcessBuilder("java", "-Xmx128M", "-cp", System.getProperty("java.class.path"),
                Repl.class.getCanonicalName(), "--sandboxed", "--ignoreConsole", "--port=" + port.get(),
                "--expressionTimeout=5", "--inactivityTimeout=300",
                expression.map(Strings.format("--expression=%s")).getOrElse(""));
  ...
  private ConsoleStatus status() {
    try {
      return ConsoleStatus.valueOf(
              parse(new ClientHttpHandler().handle(get("http://localhost:" + port.get() + "/status").build()).entity()
                      .toString()).get("status", String.class));
  ...
  public Response execute(String expression) {
    ...
    try {
      return reportProcessError(new ClientHttpHandler()
              .handle(RequestBuilder.post("http://localhost:" + port.get() + "/" + "execute")
                      .form("expression", expression).build()));
    }
    catch (Exception e) {
      e.printStackTrace();
  ...
  public Response readExpression(String line) {
    ...
    try {
      return reportProcessError(new ClientHttpHandler()
              .handle(RequestBuilder.post("http://localhost:" + port.get() + "/" + "readExpression").form("line", line)
                      .build()));
    }
    catch (Exception e) {
      e.printStackTrace();
  ...
  public Response completions(String expression) {
    ...
    try {
      return reportProcessError(new ClientHttpHandler()
              .handle(RequestBuilder.get("http://localhost:" + port.get() + "/" + "completions")
                      .query("expression", expression).build()));
    }
    catch (Exception e) {
      e.printStackTrace();
      return response(INTERNAL_SERVER_ERROR);
