...
public class Utils {
    ...
    if (type instanceof Class) {
      ...
      if (clazz.isAnonymousClass() || clazz.isSynthetic()) {
        if (clazz.getGenericSuperclass().equals(Object.class)) {
          return extractType(sequence(clazz.getGenericInterfaces()).headOption().getOrElse(Object.class));
        }
        else {
  ...
  public static String applicationVersion() {
    ...
    try {
      File path =
              new File(decode(Main.class.getProtectionDomain().getCodeSource().getLocation().getPath(), "ISO-8859-1"));
      if (!path.isDirectory()) {
  ...
  private static <T> Sequence<Sequence<T>> cartesianProductPower(Sequence<T> items, int times) {
    ...
    return cartesianProductPower(items, times - 1).cartesianProduct(items)
  ...
  public static Sequence<String> entries(File file) {
    if (file.isDirectory()) {
      return recursiveFiles(file).map(path().then(replace(file.getPath() + File.separator, "")));
    }
    else {
      try {
        return memorise(new JarFile(new File(file.toURI())).entries()).map(jarEntryName());
      }
      catch (Exception e) {
